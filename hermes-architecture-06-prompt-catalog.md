# Hermes 架构解析 (六)：提示词全谱篇 -- 系统提示词完整目录与设计解析 (v2026.4.22)
...
### A1. DEFAULT_AGENT_IDENTITY
- **位置**: `agent/prompt_builder.py:134-142`
...
### A2. SOUL.md (用户自定义身份)
- **位置**: `agent/prompt_builder.py:906-931` (`load_soul_md()`)
...
### A3. CLI 个性模式 (Personalities)
- **位置**: `cli.py:324-339` (CLI_CONFIG 中的 `personalities` 字典)
...
### B1. MEMORY_GUIDANCE
- **位置**: `agent/prompt_builder.py:144-162`
...
- **位置**: `agent/prompt_builder.py:164-168`
...
### B3. SKILLS_GUIDANCE
- **位置**: `agent/prompt_builder.py:170-177`
...
### B4. TOOL_USE_ENFORCEMENT_GUIDANCE
- **位置**: `agent/prompt_builder.py:179-192`
...
### B5. OPENAI_MODEL_EXECUTION_GUIDANCE
- **位置**: `agent/prompt_builder.py:202-260`
...
### B6. GOOGLE_MODEL_OPERATIONAL_GUIDANCE
- **位置**: `agent/prompt_builder.py:264-282`
...
### B7. DEVELOPER_ROLE_MODELS (角色切换)
- **位置**: `agent/prompt_builder.py:289`
- **值**: `("gpt-5", "codex")`
...
### C1. PLATFORM_HINTS (13 个平台)
- **位置**: `agent/prompt_builder.py:291-399`
...
### C2. WSL_ENVIRONMENT_HINT
- **位置**: `agent/prompt_builder.py:407-416`
...
### D1. 技能索引块 (Skills Index Block)
- **位置**: `agent/prompt_builder.py:790-812` (`build_skills_system_prompt()`)
...
### D2. 项目上下文文件 (Context Files)
- **位置**: `agent/prompt_builder.py:1019-1058` (`build_context_files_prompt()`)
...
### D3. Nous 订阅提示 (Nous Subscription Prompt)
- **位置**: `agent/prompt_builder.py:824-887` (`build_nous_subscription_prompt()`)
...
### D4. 时间戳与模型身份行 (Timestamp / Model Identity)
- **位置**: `run_agent.py:4105-4126`
- **触发条件**: 始终附加到系统提示词末尾
...
### E1. terminal (终端工具)
- **位置**: `tools/terminal_tool.py:700-720`
...
### F1. Memory Flush Prompt (记忆冲洗)
- **位置**: `run_agent.py:7292-7296`
...
### F2. Max Iterations Summary Request (迭代上限摘要)
- **位置**: `run_agent.py:8396-8400`
- **触发条件**: 工具调用迭代预算耗尽时
- **内容**:
...
### F3. Length Continuation Message (长度续写)
- **位置**: `run_agent.py:9689-9695`
...
### F4. Codex Intermediate Ack Continuation (Codex 中间确认续写)
- **位置**: `run_agent.py:11596-11602`
...
### F5. Post-Tool Empty Response Nudge (工具后空响应推动)
- **位置**: `run_agent.py:11439-11446`
- **触发条件**: 模型执行工具调用后返回空内容 (首次出现，仅允许一次推动)
- **内容**:

> You just executed tool calls but returned an empty response. Please process the tool results above and continue with the task.

### F6. Stub Tool Result (孤立工具结果占位)
- **位置**: `run_agent.py:4212`
- **触发条件**: `_sanitize_api_messages()` 中修复压缩后丢失结果的工具调用
- **内容**: `"[Result unavailable -- see context summary above]"`

### F7. Tool Cancelled / Skipped Messages (工具取消消息)
- **位置**: `run_agent.py:7719, 8020-8023, 8366-8373`
- **触发条件**: 用户中断工具执行
- **内容**:
...
### F8. 上下文文件注入攻击扫描 (_CONTEXT_THREAT_PATTERNS)
- **位置**: `agent/prompt_builder.py:36-73`
...
### F9. User Guidance (Steer) Injection (用户引导注入)
- **位置**: `run_agent.py:3719`
- **触发条件**: 用户在工具执行期间发送引导文本 (不中断)
- **内容**: `"\n\nUser guidance: {steer_text}"` 附加到最后的工具结果
- **设计意图**: 允许用户在不打断工具执行的情况下微调 Agent 行为。

### F10. 上下文文件截断标记
- **位置**: `agent/prompt_builder.py:894-903`
...
### F11. Thinking Budget Exhausted (思考预算耗尽)
- **位置**: `run_agent.py:9656-9663`
...
### G1. _MEMORY_REVIEW_PROMPT (记忆审查)
- **位置**: `run_agent.py:2761-2770`
...
### G2. _SKILL_REVIEW_PROMPT (技能审查)
- **位置**: `run_agent.py:2772-2779`
...
### G3. _COMBINED_REVIEW_PROMPT (联合审查)
- **位置**: `run_agent.py:2782-2794`
- **触发条件**: 记忆审查和技能审查同时到期
...
### H1. 压缩器: SUMMARY_PREFIX (摘要前缀)
- **位置**: `agent/context_compressor.py:38-49`
- **触发条件**: 每次压缩摘要注入对话时前置
...
### H2. 压缩器: Summarizer Preamble (摘要器指令)
- **位置**: `agent/context_compressor.py:635-648`
...
### H3. 压缩器: Structured Summary Template (结构化摘要模板)
- **位置**: `agent/context_compressor.py:651-708`
- **触发条件**: 首次和后续压缩
- **模板章节**:
...
### H4. 压缩器: Iterative Update Prompt (迭代更新指令)
- **位置**: `agent/context_compressor.py:712-724`
...
### H5. 压缩器: First Compaction Prompt (首次压缩指令)
- **位置**: `agent/context_compressor.py:727-736`
- **触发条件**: 会话中首次压缩
- **关键句**: "Create a structured handoff summary for a different assistant that will continue this conversation"

### H6. 压缩器: Focus Topic Guidance (焦点主题)
- **位置**: `agent/context_compressor.py:741-744`
...
### H7. 压缩器: Compression Note (压缩注释)
- **位置**: `agent/context_compressor.py:1148`
- **触发条件**: 压缩过程中附加到系统提示词
- **内容**: `"[Note: Some earlier conversation turns have been compacted into a handoff summary...]"`

### H8. 压缩器: Summary Merge Separator (摘要合并分隔符)
- **位置**: `agent/context_compressor.py:1197-1198`
- **内容**: `"--- END OF CONTEXT SUMMARY -- respond to the message below, not the summary above ---"`

### H9. 压缩器: Static Fallback Summary (静态降级摘要)
- **位置**: `agent/context_compressor.py:1159-1165`
- **触发条件**: 摘要生成完全失败时
- **内容**: `"Summary generation was unavailable. {n_dropped} conversation turns were removed..."`

### H10. 压缩器: Stub Tool Result (工具结果存根)
- **位置**: `agent/context_compressor.py:896`
...
### H11. 记忆管理器: Memory Context Fence Block (记忆上下文围栏)
- **位置**: `agent/memory_manager.py:74-80`
...
### H12. 子目录提示: Subdirectory Context Discovery (子目录上下文发现)
- **位置**: `agent/subdirectory_hints.py:215`
- **触发条件**: Agent 首次在含 AGENTS.md/CLAUDE.md/.cursorrules 的子目录中操作
- **内容**: `"[Subdirectory context discovered: {rel_path}]\n{content}"`

### H13. 技能命令: Skill Activation Note (技能激活注释)
- **位置**: `agent/skill_commands.py:454-456`
...
### H14. 技能命令: Preloaded Skill Note (预加载技能注释)
- **位置**: `agent/skill_commands.py:493-496`
...
### H15. 技能命令: Skill Directory Path Instruction (技能目录路径)
- **位置**: `agent/skill_commands.py:265-269`
...
### H16. 技能命令: Skill Config Injection (技能配置注入)
- **位置**: `agent/skill_commands.py:225-229`
- **触发条件**: 技能前置元数据声明 `metadata.hermes.config`
- **格式**: `"[Skill config (from ~/.hermes/config.yaml): key = value ...]"`

### H17. 技能命令: 各类状态注释 (Status Notes)
- **位置**: `agent/skill_commands.py:276-451`
- **包含 7 种注释**:
...
### H18. 网关: Session Flush Prompt (会话冲洗)
- **位置**: `gateway/run.py:947-972`
...
### H19. 网关: BOOT.md Startup Prompt (启动清单)
- **位置**: `gateway/builtin_hooks/boot_md.py:30-42`
...
### H20. 网关: Channel Prompt (频道提示词)
- **位置**: `gateway/platforms/base.py:721-723, 849-863`
- **触发条件**: 配置了 `channel_prompts` 的 Discord/其他频道
- **设计意图**: 允许为不同 Discord 频道设置不同的 `ephemeral_system_prompt`，实现频道级行为定制。

### H21. Slack: Thread Context Injection (线程上下文注入)
- **位置**: `gateway/platforms/slack.py:1497-1499`
...
### H22. 定时任务: Cron Job System Hint
- **位置**: `cron/scheduler.py:645-655`
- **触发条件**: 每次 cron 任务执行前
...
### H23. 定时任务: Script Output Injection (脚本输出注入)
- **位置**: `cron/scheduler.py:622-641`
- **触发条件**: cron 任务配置了 `script` 字段
- **成功**: `"## Script Output\nThe following data was collected by a pre-run script. Use it as context.\n{output}"`
- **失败**: `"## Script Error\nThe data-collection script failed. Report this to the user.\n{output}"`

### H24. 定时任务: Cron Skill Loading (cron 技能加载)
- **位置**: `cron/scheduler.py:681-698`
...
### H25. 子代理: Child Agent System Prompt (子代理系统提示词)
- **位置**: `tools/delegate_tool.py:239-312`
...
### H26. 多模型协作: MoA Aggregator System Prompt
- **位置**: `tools/mixture_of_agents_tool.py:82-84`
...
### H27. 网页工具: Web Extract Summarizer Prompts (网页摘要器)
- **位置**: `tools/web_tools.py:602-797`
...
### H28. 会话搜索: Session Summarization Prompts
- **位置**: `tools/session_search_tool.py:200-220`
- **触发条件**: 关键词搜索模式查找过去会话
...
### H29. 视觉工具: Vision Analysis Full Prompt
- **位置**: `tools/vision_tools.py:778-781`
- **触发条件**: `vision_analyze` 工具调用
- **内容**: `"Fully describe and explain everything about this image, then answer the following question:\n\n{question}"`

### H29b. 浏览器工具: Content Extraction Prompt (内容提取)
- **位置**: `tools/browser_tool.py:1306-1317`
...
### H29c. 浏览器工具: Browser Vision Prompt (浏览器视觉分析)
- **位置**: `tools/browser_tool.py:2086-2093`
...
### H30. Anthropic OAuth: Claude Code System Prefix
- **位置**: `agent/anthropic_adapter.py:206`
...
### H31. Codex Auxiliary Default Instruction
- **位置**: `agent/auxiliary_client.py:362`
- **触发条件**: 辅助客户端通过 Codex Responses API 路由且无系统消息
- **内容**: `"You are a helpful assistant."`

### H32. RL CLI: RL_SYSTEM_PROMPT (强化学习工程师)
- **位置**: `rl_cli.py:113-170`
...
### H33. SWE Environment: System Prompt
- **位置**: `environments/hermes_swe_env/hermes_swe_env.py:92-96`
...
### H34. Web Research Environment: System Prompt
- **位置**: `environments/web_research_env.py:229-234`
...
### H35. Web Research Environment: Format Prompt
- **位置**: `environments/web_research_env.py:328-339`
- **触发条件**: 每个研究问题的用户消息生成
...
### H36. Web Research Environment: Judge Prompt (LLM 裁判)
- **位置**: `environments/web_research_env.py:623-636`
- **触发条件**: RL 奖励计算时评估答案质量
...
### H36b. Agentic OPD Environment: Hint Judge Prompt (过程奖励模型)
- **位置**: `environments/agentic_opd_env.py:221-241`
- **变量**: `_HINT_JUDGE_SYSTEM`
...
### H36c. Agentic OPD Environment: System Prompt
- **位置**: `environments/agentic_opd_env.py:406-413`
...
### H36d. Terminal Test Environment: System Prompt
- **位置**: `environments/terminal_test_env/terminal_test_env.py:126-129`
...
### H37. Mini-SWE Runner: System Prompt
- **位置**: `mini_swe_runner.py:438-448`
...
### H38. Mini-SWE Runner: Trajectory System Message
- **位置**: `mini_swe_runner.py:322-333`
- **触发条件**: 保存训练数据时 (非发送给实时 LLM)
- **内容**: `"You are a function calling AI model... <tools>...</tools> XML tags..."` -- Hermes 工具调用训练格式。

### H39. Trajectory Format (run_agent.py)
- **位置**: `run_agent.py:3046-3058`
- **触发条件**: `save_trajectories=True` 时保存训练数据
- **内容**: 与 H38 相同的 XML 工具调用格式

### H40. MCP Server: Instructions
- **位置**: `mcp_serve.py:439-445`
...
### H41. Ephemeral System Prompt (通用入口)
- **位置**: `run_agent.py:9100-9101` (Agent), `cli.py:1904-1907` (CLI), `gateway/run.py:1309` (Gateway)
- **触发条件**: `HERMES_EPHEMERAL_SYSTEM_PROMPT` 环境变量或 `config.yaml` 配置
- **设计意图**: 通用用户自定义提示词注入点，仅 API 调用时生效，永远不持久化。

### H42. Prefill Messages (预填充消息)
- **位置**: `run_agent.py:9111-9114`
- **触发条件**: 调用者提供 `prefill_messages` 列表
- **设计意图**: 在系统提示词后注入 few-shot 示例或响应风格引导。

### H43. Plugin-Injected User Context (插件注入上下文)
- **位置**: `run_agent.py:9055-9066`
...
| 编号 | 名称 | 文件 | 行号 | 发送对象 | 触发条件 |
|------|------|------|------|----------|----------|
| A1 | DEFAULT_AGENT_IDENTITY | prompt_builder.py | 134-142 | 主 LLM (系统) | 无 SOUL.md |
| A2 | SOUL.md | prompt_builder.py | 906-931 | 主 LLM (系统) | 文件存在 |
| A3 | Personalities | cli.py | 324-339 | 主 LLM (临时) | /personality 命令 |
| B1 | MEMORY_GUIDANCE | prompt_builder.py | 144-162 | 主 LLM (系统) | memory 工具加载 |
| B2 | SESSION_SEARCH_GUIDANCE | prompt_builder.py | 164-168 | 主 LLM (系统) | session_search 加载 |
| B3 | SKILLS_GUIDANCE | prompt_builder.py | 170-177 | 主 LLM (系统) | skill_manage 加载 |
| B4 | TOOL_USE_ENFORCEMENT | prompt_builder.py | 179-192 | 主 LLM (系统) | 匹配模型族 |
| B5 | OPENAI_EXECUTION | prompt_builder.py | 202-260 | 主 LLM (系统) | GPT/Codex |
| B6 | GOOGLE_OPERATIONAL | prompt_builder.py | 264-282 | 主 LLM (系统) | Gemini/Gemma |
| B7 | DEVELOPER_ROLE | prompt_builder.py | 289 | (角色切换) | GPT-5/Codex |
| C1 | PLATFORM_HINTS x13 | prompt_builder.py | 291-399 | 主 LLM (系统) | 平台匹配 |
| C2 | WSL_HINT | prompt_builder.py | 407-416 | 主 LLM (系统) | WSL 检测 |
| D1 | Skills Index | prompt_builder.py | 790-812 | 主 LLM (系统) | 技能存在 |
| D2 | Context Files | prompt_builder.py | 1019-1058 | 主 LLM (系统) | 项目文件存在 |
| D3 | Nous Subscription | prompt_builder.py | 824-887 | 主 LLM (系统) | Nous 启用 |
| D4 | Timestamp/Model ID | run_agent.py | 4105-4126 | 主 LLM (系统) | 始终 |
| E1 | terminal 描述 | terminal_tool.py | 700-720 | 主 LLM (工具) | 工具加载 |
| E2 | memory 描述 | memory_tool.py | 515-537 | 主 LLM (工具) | 工具加载 |
| E3 | session_search 描述 | session_search_tool.py | 529-552 | 主 LLM (工具) | 工具加载 |
| E4 | skill_manage 描述 | skill_manager_tool.py | 688-707 | 主 LLM (工具) | 工具加载 |
| E5 | skill_view/list 描述 | skills_tool.py | 1387-1402 | 主 LLM (工具) | 工具加载 |
| E6 | delegate_task 描述 | delegate_tool.py | 1508-1540 | 主 LLM (工具) | 工具加载 |
| E7 | execute_code 描述 | code_execution_tool.py | 1517-1537 | 主 LLM (工具) | 工具加载 |
| E8 | web_search/extract 描述 | web_tools.py | 2048-2077 | 主 LLM (工具) | 工具加载 |
| E9 | vision_analyze 描述 | vision_tools.py | 756-757 | 主 LLM (工具) | 工具加载 |
| E10 | mixture_of_agents 描述 | mixture_of_agents.py | 516-517 | 主 LLM (工具) | 工具加载 |
| E11 | cronjob 描述 | cronjob_tools.py | 390-406 | 主 LLM (工具) | 工具加载 |
| E12 | browser_vision | browser_camofox.py | 541 | 辅助 LLM | 浏览器截图 |
| E13 | Memory Headers | memory_tool.py | 391-407 | 主 LLM (系统) | 记忆存在 |
| E14 | Plugin Skill Banner | skills_tool.py | 799-806 | 主 LLM (工具) | 插件技能 |
| F1 | Memory Flush | run_agent.py | 7292-7296 | 主 LLM (用户) | 压缩前/退出前 |
| F2 | Max Iterations | run_agent.py | 8396-8400 | 主 LLM (用户) | 预算耗尽 |
| F3 | Length Continue | run_agent.py | 9689-9695 | 主 LLM (用户) | 长度截断 |
| F4 | Codex Ack Continue | run_agent.py | 11596-11602 | 主 LLM (用户) | Codex 规划响应 |
| F5 | Empty Nudge | run_agent.py | 11439-11446 | 主 LLM (用户) | 空响应 |
| F6 | Stub Tool Result | run_agent.py | 4212 | 主 LLM (工具) | 孤立修复 |
| F7 | Tool Cancelled | run_agent.py | 7719+ | 主 LLM (工具) | 用户中断 |
| F8 | Injection Scan | prompt_builder.py | 36-73 | (阻断) | 文件加载 |
| F9 | Steer Injection | run_agent.py | 3719 | 主 LLM (工具) | /steer 命令 |
| F10 | Truncation Marker | prompt_builder.py | 894-903 | 主 LLM (系统) | 文件超限 |
| F11 | Thinking Exhausted | run_agent.py | 9656-9663 | (返回用户) | 推理耗尽 |
| G1 | Memory Review | run_agent.py | 2761-2770 | 后台 LLM | N 轮间隔 |
| G2 | Skill Review | run_agent.py | 2772-2779 | 后台 LLM | N 次工具间隔 |
| G3 | Combined Review | run_agent.py | 2782-2794 | 后台 LLM | 同时触发 |
| H1 | SUMMARY_PREFIX | context_compressor.py | 38-49 | 主 LLM (注入) | 压缩 |
| H2 | Summarizer Preamble | context_compressor.py | 635-648 | 辅助 LLM | 生成摘要 |
| H3 | Summary Template | context_compressor.py | 651-708 | 辅助 LLM | 生成摘要 |
| H4 | Iterative Update | context_compressor.py | 712-724 | 辅助 LLM | 第 2+ 次压缩 |
| H5 | First Compaction | context_compressor.py | 727-736 | 辅助 LLM | 首次压缩 |
| H6 | Focus Topic | context_compressor.py | 741-744 | 辅助 LLM | /compress topic |
| H7 | Compression Note | context_compressor.py | 1148 | 主 LLM (系统) | 压缩中 |
| H8 | Merge Separator | context_compressor.py | 1197-1198 | 主 LLM (注入) | 摘要合并 |
| H9 | Fallback Summary | context_compressor.py | 1159-1165 | 主 LLM (注入) | 摘要失败 |
| H10 | Stub Result | context_compressor.py | 896 | 主 LLM (工具) | 孤立修复 |
| H11 | Memory Fence | memory_manager.py | 74-80 | 主 LLM (用户) | API 调用 |
| H12 | Subdir Discovery | subdirectory_hints.py | 215 | 主 LLM (工具) | 子目录访问 |
| H13 | Skill Activation | skill_commands.py | 454-456 | 主 LLM (用户) | /skill 命令 |
| H14 | Preloaded Skill | skill_commands.py | 493-496 | 主 LLM (用户) | --skill 启动 |
| H15 | Skill Dir Path | skill_commands.py | 265-269 | 主 LLM (用户) | 技能加载 |
| H16 | Skill Config | skill_commands.py | 225-229 | 主 LLM (用户) | 有配置的技能 |
| H17 | Status Notes x7 | skill_commands.py | 276-451 | 主 LLM (用户) | 各种状态 |
| H18 | Session Flush | gateway/run.py | 947-972 | 临时 LLM | 会话过期 |
| H19 | BOOT.md | boot_md.py | 30-42 | 临时 LLM | 网关启动 |
| H20 | Channel Prompt | base.py | 721-863 | 主 LLM (临时) | 频道配置 |
| H21 | Thread Context | slack.py | 1497-1499 | 主 LLM (用户) | Slack @提及 |
| H22 | Cron Hint | scheduler.py | 645-655 | 主 LLM (用户) | cron 执行 |
| H23 | Script Output | scheduler.py | 622-641 | 主 LLM (用户) | cron script |
| H24 | Cron Skill Load | scheduler.py | 681-698 | 主 LLM (用户) | cron skills |
| H25 | Child Agent | delegate_tool.py | 239-312 | 子 LLM (系统) | 委派 |
| H26 | MoA Aggregator | mixture_of_agents.py | 82-84 | 聚合 LLM | MoA 合成 |
| H27 | Web Summarizer x6 | web_tools.py | 602-797 | 辅助 LLM | 网页提取 |
| H28 | Session Summary x2 | session_search.py | 200-220 | 辅助 LLM | 会话搜索 |
| H29 | Vision Full | vision_tools.py | 778-781 | 视觉 LLM | 视觉分析 |
| H29b | Browser Content Extract | browser_tool.py | 1306-1317 | 辅助 LLM | 页面快照提取 |
| H29c | Browser Vision | browser_tool.py | 2086-2093 | 视觉 LLM | 浏览器截图 |
| H30 | Claude Code Prefix | anthropic_adapter.py | 206 | (系统前缀) | OAuth |
| H31 | Codex Aux Default | auxiliary_client.py | 362 | 辅助 LLM | 无系统消息 |
| H32 | RL System | rl_cli.py | 113-170 | 主 LLM (临时) | RL CLI |
| H33 | SWE Env System | hermes_swe_env.py | 92-96 | RL LLM | SWE 环境 |
| H34 | Web Research System | web_research_env.py | 229-234 | RL LLM | 研究环境 |
| H35 | Web Research Format | web_research_env.py | 328-339 | RL LLM (用户) | 问题格式化 |
| H36 | Web Research Judge | web_research_env.py | 623-636 | 裁判 LLM | 奖励计算 |
| H36b | OPD Hint Judge | agentic_opd_env.py | 221-241 | 裁判 LLM | 回溯提示 |
| H36c | OPD System | agentic_opd_env.py | 406-413 | RL LLM | OPD 环境 |
| H36d | Terminal Test System | terminal_test_env.py | 126-129 | RL LLM | 终端测试环境 |
| H37 | Mini-SWE System | mini_swe_runner.py | 438-448 | 主 LLM (临时) | Mini-SWE |
| H38 | Mini-SWE Trajectory | mini_swe_runner.py | 322-333 | (训练数据) | 导出轨迹 |
| H39 | Agent Trajectory | run_agent.py | 3046-3058 | (训练数据) | 导出轨迹 |
| H40 | MCP Instructions | mcp_serve.py | 439-445 | MCP 客户端 | 服务器创建 |
| H41 | Ephemeral SP | run_agent.py + cli.py + gateway | 各处 | 主 LLM (临时) | 用户配置 |
| H42 | Prefill Messages | run_agent.py | 9111-9114 | 主 LLM (对话) | 调用者提供 |
| H43 | Plugin Context | run_agent.py | 9055-9066 | 主 LLM (用户) | 插件/记忆 |
